...
exports.changeEmail = catchAsync(async (req, res, next) => {
  ...
  console.log("This is  authController: changeEmail: ", user);
  if (!user) {
  ...
  const emailToBeSentUser = {
    username: user.username,
    email: req.body.email
  }
  await new Email(emailToBeSentUser, verifyEmailURL).sendVerifyEmail();
  ...
  console.log(`Verification email sent to ${req.body.email} successfully`);
});

// verify user based on token sent to email for email verification
exports.validEmailToken = catchAsync(async (req, res, next) => {
  console.log("This is  authController: validEmailToken: ", req.body);
  const token = req.params.token;
  console.log({ token });
  if (!token) {
    return res.status(500).json({ message: "Token is required" });
  ...
  console.log(`Token verified for email verification for ${user.email} successfully`);

  const newEmail = user.newEmail;
  if (!newEmail) {
    return res.status(500).json({ message: "New email is required" });
  }
